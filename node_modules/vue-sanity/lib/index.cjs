...
function useSanityFetcher(query, initialValue = null, mapper = (result) => result, queryOptions) {
  ...
  if (!client)
    throw new Error("You must call useSanityClient before using sanity resources in this project.");
  const computedQuery = typeof query === "string" ? minifier__default["default"](query).replace(/\n/g, " ") : vueDemi.computed(() => minifier__default["default"](query() || "").replace(/\n/g, " "));
...
function useSanityClient(config, supportPreview = false, defaultOptions = {}) {
  ensureInstance();
  const client = sanityClient__default["default"](config);
  const imageBuilder = imageUrlBuilder__default["default"](config);
  ...
  if (supportPreview) {
    const previewClient = sanityClient__default["default"]({
      ...config,
      useCdn: false,
      token: void 0,
